...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl border border-white/20 bg-white/90 p-3 shadow-lg backdrop-blur-md">
        <p className="mb-1 text-sm font-semibold text-secondary-900">{label}</p>
  ...
  return (
    <Card className="h-[400px] w-full border-none shadow-sm flex flex-col p-5">
      <div className="mb-6 flex items-center justify-between">
        <h3 className="text-lg font-bold tracking-tight text-secondary-900">Signups Trend</h3>
        <span className="rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold text-primary-600">Last 30 Days</span>
      </div>
...
export function VehicleTypeChart({ data }: { data: Record<string, number> }) {
  ...
  return (
    <Card className="h-[320px] w-full flex flex-col border-none shadow-sm p-5">
      <h3 className="mb-2 text-base font-bold tracking-tight text-secondary-900">Vehicle Types</h3>
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
...
export function LanguageChart({ data }: { data: Record<string, number> }) {
  ...
  return (
    <Card className="h-[320px] w-full flex flex-col border-none shadow-sm p-5">
      <h3 className="mb-2 text-base font-bold tracking-tight text-secondary-900">Language Preferences</h3>
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={chartData} cx="50%" cy="50%" innerRadius={0} outerRadius={85} dataKey="value">
              {chartData.map((_entry, index) => (
                <Cell key={`cell-${index}`} fill={COLORS[(index + 2) % COLORS.length]} stroke="#fff" strokeWidth={2} />
              ))}
            </Pie>
            <Tooltip content={<CustomTooltip />} />
            <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ fontSize: '12px', fontWeight: 500 }} />
...
export function CityChart({ data }: { data: Record<string, number> }) {
  ...
  return (
    <Card className="h-[320px] w-full flex flex-col border-none shadow-sm p-5">
      <h3 className="mb-4 text-base font-bold tracking-tight text-secondary-900">Top Cities</h3>
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
...
export function PlatformChart({ data }: { data: Record<string, number> }) {
  ...
  return (
    <Card className="h-[320px] w-full flex flex-col border-none shadow-sm p-5">
      <h3 className="mb-4 text-base font-bold tracking-tight text-secondary-900">Delivery Platforms</h3>
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
